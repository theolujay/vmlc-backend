...
class UserVerificationUploadView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            logger.info(
                "Verification data submitted by user %s.",
                request.user.id
            )
            return Response({"detail": "Documents uploaded successfully."}, status=status.HTTP_200_OK)
    ...
    def patch(self, request):
        ...
        if serializer.is_valid():
            with transaction.atomic():
                ...
                logger.info(
                    "Verfication data updated by user %s.",
                    request.user.id
                )
                ...
                return Response(
                    {
                        ...
                        "verification_data": UserVerificationUploadSerializer(verification).data,
                    },
    ...
    def get(self, request, file_type, user_id=None):
        ...
        if user_id is None:
            ...
            if (request.user.id != target_user.id and 
                not IsObjectOwnerOrSuperAdminRole().has_object_permission(request, self, target_user)):
                raise PermissionDenied("You don't have permission to access this user's documents.")
            
            verification = get_object_or_404(UserVerification, user=target_user)
            
        if user_id and request.user.id != user_id:
            logger.info(f"Superadmin {request.user.id} accessing {file_type} for user {user_id}")
