...
def get_registration_metrics(days=30, weeks=12):
    ...
    daily_users = (
        User.objects.filter(date_joined__gte=start_date_daily)
    ...
    daily_candidates = (
        Candidate.objects.filter(created_at__gte=start_date_daily)
    ...
    daily_staff = (
        Staff.objects.filter(created_at__gte=start_date_daily)
        .annotate(day=TruncDay("created_at"))
        .values("day")
        .annotate(count=Count("user_id"))
        .order_by("day")
    ...
    daily_pre_reg = (
        PreRegUser.objects.filter(created_at__gte=start_date_daily)
        .annotate(day=TruncDay("created_at"))
        .values("day")
        .annotate(count=Count("id"))
        .order_by("day")
    ...
    weekly_users = (
        User.objects.filter(date_joined__gte=start_date_weekly)
    ...
    weekly_candidates = (
        Candidate.objects.filter(created_at__gte=start_date_weekly)
    ...
    weekly_staff = (
        Staff.objects.filter(created_at__gte=start_date_weekly)
        .annotate(week=TruncWeek("created_at"))
        .values("week")
        .annotate(count=Count("user_id"))
        .order_by("week")
    ...
    weekly_pre_reg = (
        PreRegUser.objects.filter(created_at__gte=start_date_weekly)
        .annotate(week=TruncWeek("created_at"))
        .values("week")
        .annotate(count=Count("id"))
        .order_by("week")
...
def get_funnel_metrics():
    ...
    def calculate_stats(
        pre_events_qs, conv_events_qs, completed_fallback=None, pending_fallback=None
    ):
    ...
    overall_stats = calculate_stats(
        ...
        pending_fallback=PreRegUser.objects.exclude(
    ...
    candidate_stats = calculate_stats(
        ...
        pending_fallback=PreRegUser.objects.filter(interest_type="candidate")
    ...
    volunteer_stats = calculate_stats(
        ...
        pending_fallback=PreRegUser.objects.filter(interest_type="volunteer")
        .exclude(email__in=registered_emails)
        .count(),
    )
