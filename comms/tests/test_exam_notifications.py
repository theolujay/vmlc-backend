...
from competition.models import Competition, Stage, StageExam, Enrollment, EnrollmentStageProgress
...
class ExamNotificationTest(TestCase):
    def setUp(self):
        ...
        self.competition = Competition.objects.create(name="Test Competition", edition=1)
    ...
    def test_notify_candidates_about_exam_calls_notify_user(self, mock_send_bulk, mock_send_mail):
        service = NotificationService()
        
        # Avoid real DB/Group calls for platform notification
        with patch.object(NotificationService, "_send_platform_notification", return_value=True):
