...
Permissions have been refactored to use the dynamic `request.enrollment` context:
- `IsInStage('stage_type')`: Grants access based on the candidate's `current_stage` in the active competition.
- `IsActiveCompetitionParticipant`: Ensures the candidate's enrollment status is `ACTIVE`.
...
- **Promotion Source**: Based on Screening `RankingSnapshot` or League `AggregateLeaderboard`.
