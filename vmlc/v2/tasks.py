...
def invalidate_exam_related_caches_task(exam_id):
    ...
    if exam.competition_slot and exam.competition_slot.competition_stage:
        ...
        logger.info(f"Invalidated caches for {len(candidates)} candidates for exam {exam_id}")
    ...
    log_event(
        ...
        metadata={
            "exam_id": str(exam_id),
            "affected_candidates_count": len(candidates) if 'candidates' in locals() else 0,
