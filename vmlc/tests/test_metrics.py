...
class RegistrationMetricsViewTest(APITestCase):
    ...
    def test_get_registration_metrics_success(self):
        ...
        Candidate.objects.create(user=user1, school_name="School 1", created_at=now - timedelta(days=1))
        
        # Events for funnel
        Event.objects.create(event_name="PRE_REGISTRATION", metadata={"interest_type": "candidate"})
        Event.objects.create(event_name="PRE_REG_CONVERSION", metadata={"interest_type": "candidate"})
