...
class ExamAccessTests(APITestCase):
    ...
    def setUp(self):
        ...
        self.slot = StageExam.objects.create(
            competition_stage=self.stage, is_active=True
        )
    ...
    @patch(
        "vmlc.services.exam_access.settings.FRONTEND_BASE_URL", "https://example.com"
    )
    ...
    def test_authenticate_passcode(self):
        ...
        access = ExamAccess.objects.create(
            exam=self.exam, candidate=self.candidate, status=ExamAccess.Status.ISSUED
        )
    ...
    def test_face_capture_and_take_exam(self):
        ...
        url_face = reverse(
            "vmlc-v2:exam-face-capture", kwargs={"exam_id": self.exam.id}
        )
        ...
        response = self.client.post(
            url_face, {"face_capture": file}, format="multipart"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
    ...
    def test_passcode_task_triggered_on_scheduled(self, mock_task):
        ...
        # Create a DRAFT exam
        exam = Exam.objects.create(description="Draft Exam", created_by=self.staff)
